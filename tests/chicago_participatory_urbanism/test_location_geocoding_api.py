...
@pytest.mark.parametrize("test_location, expected_result_type", [
    ...
    ('ON N LEAVITT ST FROM W DIVISION ST (1200 N) TO W NORTH AVE (1600 N)', LineString),
    ...
    ('ON N KOLMAR AVE FROM W CORNELIA AVE (3500 N) TO W ADDISON ST (3600 N)', LineString),
    ('200-250 E 40TH ST', LineString),  # unable to find addresss
    ('N WHIPPLE ST & W BLOOMINGDALE AVE & N HUMBOLDT BLVD W & W CORTLAND ST', Polygon),
    ('1400 N CAMPBELL AVE; N CAMPBELL AVE & W LE MOYNE ST & W HIRSCH ST & N MAPLEWOOD AVE', Polygon),
    ('6754 S EUCLID AVE; ON W 68TH ST FROM S BENNETT AVE  (1900 E) TO S EUCLID AVE  (1930 E)', GeometryCollection),
    ('1110 N STATE ST; 1030 N STATE ST', MultiPoint),
    ...
    ('ON N HONORE ST FROM N WICKER PARK AVE (1500 N) TO N MILWAUKEE AVE (1520 N)', LineString),
    ('ON W DELAWARE PL FROM N DEARBORN ST (40 W) TO N CLARK ST (100 W)', LineString),
    ('S DR MARTIN LUTHER KING JR DR & E 42ND ST', Point),
    ("ON W BARRY AVE FROM N BROADWAY ST (599 W) TO N CLARK ST (800 W)", LineString),
    ("ON W BRIAR PL FROM N BROADWAY ST (599 W) TO N HALSTED ST (800 W)", LineString),
    ("ON W OAKDALE AVE FROM N CLARK ST (700 W) TO N PINE GROVE AVE (500 W)", LineString),
    ("ON W SURF ST FROM N PINE GROVE AVE (510 W) TO N CLARK ST (700 W)", LineString),
    ("ON W SURF ST FROM N PINE GROVE AVE (510 W) TO N SHERIDAN RD (400 W)", LineString),
    ("ON W WELLINGTON AVE FROM N BROADWAY ST (599 W) TO N CLARK ST (730 W)", LineString),
    ("ON W WELLINGTON ST FROM N BROADWAY ST (599 W) TO N CLARK ST (730 W)", LineString),
...
@pytest.mark.parametrize("test_location", [
    '1400 N WRONG AVE',
    'jkdfhgjkoahfiea',
