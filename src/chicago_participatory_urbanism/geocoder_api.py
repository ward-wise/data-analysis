...
class GeoCoderAPI:
    ...
    def _query_transport_api(self, params: Dict[str, str], sql_func: Optional[str] = None) -> Optional[np.array]:
        # https://data.cityofchicago.org/Transportation/Street-Center-Lines/6imu-meau
        # https://dev.socrata.com/foundry/data.cityofchicago.org/pr57-gg9e
        ...
        query_params = " AND ".join(k + " like " + "'" + str(v).upper() + "'" for k, v in params.items())
    ...
    def _query_address_api(self, params: Dict[str, str], sql_func: Optional[str] = None) -> Optional[np.array]:
        """
        useful fields:
    ...
    def _query_nominatim(self, query_string: str) -> np.array:
        ...
        if len(resp.json()) == 0:
            ...
            coordinate = np.array([float(resp.json()[0]["lon"]), float(resp.json()[0]["lat"])])
            return coordinate
    ...
    def _query_census_api(self, query_string: str) -> Optional[np.array]:
        ...
        if len(street_match) == 0:
            ...
            coordinates = np.array([float(coord["x"]), float(coord["y"])])

        return coordinates

    def get_street_address_coordinates(self, address: str) -> Optional[Point]:
        ...
        results = self._query_census_api(query_string=str(address).upper())
        if results is not None and results.any():
            return Point(results)

        results = self._query_nominatim(query_string=str(address).upper())
        if results is not None and results.any():
            return Point(results)
        else:
            return None

    def get_intersection_coordinates(self, intersection: Intersection) -> Optional[Point]:
        """
        ...
        result = self._query_transport_api(params={"street_nam": street_1}, sql_func=f'f_cross like "%25{street_2}%25"')
        if result is not None and result.any():
            return Point(result)

        result = self._query_transport_api(params={"street_nam": street_2}, sql_func=f't_cross like "%25{street_1}%25"')
        if result is not None and result.any():
            return Point(result)

        result = self._query_census_api(query_string=f"{street_1} AND {street_2}")
        if result is not None and result.any():
            return Point(result)
